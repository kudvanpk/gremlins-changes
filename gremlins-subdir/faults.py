...
def kill_daemons(daemons, signal, restart_after):
  ...
  def do():
    ...
    for daemon in daemons:
      pids = procutils.find_container_pids(daemon, 'process')
      if pids:
        for pid in pids:
...
def pause_daemons(jvm_names, seconds):
  ...
  def do():
    ...
    for jvm_name in jvm_names:
      pids = procutils.find_container_pids(jvm_name, 'process')
      if not pids:
        logging.warn("No pid founds for %s" % jvm_name)
        continue
      for pid in pids:
...
def drop_packets_to_daemons(daemons, seconds):
  ...
  def do():
    ...
    all_ports = []
    for daemon in daemons:
      pid = procutils.find_container_pids(daemon, "network")
      if not pid:
        logging.warn("Daemon %s not running!" % daemon)
        continue
      ports = procutils.get_listening_ports_container(daemon)
